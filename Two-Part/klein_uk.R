...
wage <- read_stata("Z://DesktopC//LUMSA//2//Data Mining//wagepan.dta")
str(wage)
...
coeftest(lm3d.ols, vcov=vcovHC(lm3d.ols, method="arellano")) # -> same as clx(lm3b, 1, wage$nr)
...
coeftest(lm3e.ols)
coeftest(lm3e.ols, vcov=vcovHC(lm3e.ols, method="arellano"))
...
coeftest(lm3e.re)
coeftest(lm3e.re, vcov=vcovHC(lm3e.re, method="arellano"))
